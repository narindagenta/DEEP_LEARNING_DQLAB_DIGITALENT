...
# ============================================================
# HARDCODED AKUN TO CATEGORY FALLBACK (for missing master entries)
# ============================================================
HARDCODED_AKUN_TO_CATEGORY = {
...
def keyword_fallback(desc: str):
    ...
    for key, akun in KEYWORD_MAP.items():
        if key in d:
            print(f"   🔎 Keyword match → '{key}' → {akun}")
            return akun
    print("   ⚠ Tidak ada keyword match")
    return None
...
def process_ngram_matching(
    ...
    for idx, row in df_trx.iterrows():
        ...
        print(f"\n[{idx+1}] 📝 Deskripsi: {desc}")

        # 1‰ Coba dengan keyword
        akun = keyword_fallback(desc)

        # 2‰ Jika gagal → pakai n-gram
        if akun is None:
            print("   ↔ Masuk mode n-gram similarity...")
            best_score = 0
            best_match = None
        ...
        if akun is None:
            ...
            print("   ❌ Akun dan Kategori tidak ditemukan setelah matching!")
        else:
            row_found = df_master.loc[df_master["Deskripsi"].str.lower() == akun.lower(), "Kategori"]

            if row_found.empty:
                # Try hardcoded map as a fallback for Kategori
                if akun in HARDCODED_AKUN_TO_CATEGORY:
                    kategori = HARDCODED_AKUN_TO_CATEGORY[akun]
                    print(f"   ✅ Fallback kategori ditemukan: {kategori}")
                else:
                    kategori = "Tidak Ditemukan"
                    print("   ❌ Kategori tidak ditemukan di master dan fallback!")
            else:
    ...
    # Filter desired_order to only include columns present in df_out
    # This prevents KeyError if a column like "Nomor" is not in the original dataframe
    columns_to_select = [col for col in desired_order if col in df_out.columns]
